...
uint16_t q3(uint8_t x, uint8_t y) {
    ...
    uint8_t flip = x ^ 0b10000001;

    uint16_t ans = (flip << 8) + y;
...
error_t q11(q11_a_t *a, q11_b_t *b){
    ...
    uint16_t *arr = (b->array);
    arr++;

    memcpy(arr, (*a).array, 50);
    return SUCCESS;
...
void *q13(uint32_t *ptr1, uint16_t *ptr2){
    ...
    else if (ptr1 > (uint32_t *)ptr2){
        return (void *)((char *)ptr2 + 5);
    }
    else{
        return (void *)((char *)ptr1 + 5);
    }
